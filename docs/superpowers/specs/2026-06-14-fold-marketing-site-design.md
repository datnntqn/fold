...
- **Imagery = cohesive neutral studio photography in uniform cards.** All imagery is royalty-free (Unsplash), self-hosted, neutral "quiet-luxury" tone, on plain studio backgrounds (so it honestly represents what the on-device segmentation model can cut out). Closet items and hero/mix/lookbook/calendar all use `object-fit: cover` in uniform cards (same aspect ratio per row) for even sizing — an earlier `mix-blend-mode` cut-out treatment was dropped because non-pure-white backgrounds showed as grey boxes. **No styled flat-lays with props, no busy backgrounds, no jewelry, no visible brand logos.** Closet trimmed to 4 strong items.
